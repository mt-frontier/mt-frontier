...
minetest.register_node("crops:melon_seed", {
	...
	on_place = function(itemstack, placer, pointed_thing)
		...
		crops.plant(pointed_thing.above, {name="crops:melon_seed"})
...
minetest.register_craftitem("crops:melon_slice", {
	...
	on_use = minetest.item_eat(1, "crops:melon_seed")
})
...
crops.melon_grow = function(pos)
	local node = minetest.get_node(pos)
	local n = node.name
	if n == "crops:melon_plant_5" then
		for face = 1, 4 do
		...
		n = string.gsub(n, "2", "3")
		n = string.gsub(n, "1", "2")
		if n == "crops:melon_seed" then
			n = "crops:melon_plant_1"
		end
...
crops.register({ name = "crops:melon_seed", properties = properties})
