...
-- wolf
	mobs:register_mob("mobs_wolf:wolf", {
		...
		--on_die = function(self, pos)
			--end,
		on_spawn = function(self) -- spawn packs up to 5 wolves
			local obs = minetest.get_objects_inside_radius(self.object:get_pos(), 4)
			if #obs >= 5 then
				return
			end
			local n = math.random()
			if n < 0.15 then
				local mob = minetest.add_entity(self.object:get_pos(), "mobs_wolf:wolf")
			end
		end,
