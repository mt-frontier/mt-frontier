...
--	print ("Mob Damage is", damage)
	...
	if damage >= 1 then
		...
		if minetest.is_player(hitter) then
			local class = classes.get_class(hitter)
			if class == "hunter" and (self.type == "animal" or self.type == "monster") then
				local xp = math.floor(damage)
				classes.change_xp(hitter, xp)
			end
		end	
