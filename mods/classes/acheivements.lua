classes.registered_acheivements = {}

function classes.register_acheivement(name, def)
	assert(type(def) == "table", "def must be a table")
	assert(def.description, "Provide a description of the Acheivment")
	if def.storage then
		def.storage = {}
	end
	classes.registered_acheivements[name] = def
end

local function has_badge(player, badgename)
...
function classes.award_badge(player, badgename, badge_desc, announce)
...
classes.register_acheivement("harvest_crops", {
			...
			if (string.match(k, "crops:") or string.match(k, "farming:")) and (minetest.get_item_group(k, "plant") > 0 or minetest.get_item_group(k, "flora") > 0) then
				minetest.override_item(k, {
					after_dig_node = function(pos, oldnode, oldmetadata, digger)
						local class = classes.get_class(digger)
						if class == "homesteader" then
							classes.change_xp(digger, 1)
						end
					end
...
classes.register_acheivement("dig_ores", {
	...
	func = function()
		...
		for i, ore in ipairs(ores) do
			minetest.override_item(ore[1], {
				after_dig_node = function(pos, oldnode, oldmetadata, digger)
					local class = classes.get_class(digger)
					if class == "prospector" then
						classes.change_xp(digger, ore[2])
					end
				end,
			})
		end
...
classes.register_acheivement("struck_gold", {
	description = "Struck Gold!",
	badge = "badge_frame.png^[colorize:#aa8800^default_gold_lump.png",
	func = function()
		local badgename = "classes:badge_struck_gold"
		minetest.override_item("default:stone_with_gold", {
			on_punch = function(pos, node, puncher, pt)
				if not has_badge(puncher, badgename) then
					classes.award_badge(puncher, badgename, "Struck Gold!", true)
				end
			end
		})
	end,
})
classes.register_acheivement("brave_or_dumb", {
	...
	func = function()
		...
		minetest.register_on_dieplayer(function(player)
			...
			local badgename = "classes:badge_brave_or_dumb"
			if not has_badge(player, badgename) then
				classes.award_badge(player, badgename, "Either Brave or Dumb (Died in first day of a life)", true)
			end
		end)
	end
...
local num_badges = 0
for name, def in pairs(classes.registered_acheivements) do
	if def.badge ~= nil then
		num_badges = num_badges + 1
		minetest.register_craftitem("classes:badge_"..name, {
...
sfinv.register_page("classes:badges", {
	title = "Badges",
...
minetest.register_chatcommand("badges", {
	...
	func = function(name, params)
		...
		minetest.show_formspec(name, "classes:badges_"..playername, formspec)
